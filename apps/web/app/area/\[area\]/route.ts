...
export async function GET(
  ...
  try {
    const res = await fetch(`${API_URL}/restaurants/areas/${params.area}/${latitude}/${longitude}`, {
      headers: {},
        })
    ...
    return NextResponse.json({ error: 'Failed to fetch restaurants by area' }, { status: 500 })
    }
}
